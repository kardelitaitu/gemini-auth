...
pub fn refreshForegroundUsageForDisplayWithApiFetchersWithPoolInitUsingApiEnabledAndPersist(
    ...
    if (!usage_api_enabled) {
        ...
        if (try auto.refreshActiveUsage(allocator, gemini_home, reg)) {
    ...
    if (batch_fetcher) |fetch_batch| batch_fetch: {
        ...
        for (reg.accounts.items, 0..) |account, idx| {
            auth_paths[idx] = try registry.accountAuthPath(auth_path_arena, gemini_home, account.account_key);
        }
        ...
        if (use_concurrent_usage_refresh) {
            ...
            runForegroundUsageRefreshWorkersSerially(allocator, gemini_home, reg, usage_fetcher, worker_results);
        }
    }
...
fn runForegroundUsageRefreshWorkersConcurrently(
    ...
    if (worker_count <= 1) {
        runForegroundUsageRefreshWorkersSerially(allocator, gemini_home, reg, usage_fetcher, results);
        return;
    }
...
fn runForegroundUsageRefreshWorkersSerially(
    ...
    for (reg.accounts.items, 0..) |_, idx| {
        foregroundUsageRefreshWorker(allocator, gemini_home, reg, idx, usage_fetcher, results);
    }
}
...
fn foregroundUsageRefreshWorker(
    ...
    const auth_path = registry.accountAuthPath(arena, gemini_home, reg.accounts.items[account_idx].account_key) catch |err| {
