...
fn runMain(init: std.process.Init.Minimal) !void {
    ...
    const needs_gemini_home = switch (cmd) {
        .version => false,
        .help => |topic| topic == .top_level,
        else => true,
    };
    const gemini_home = if (needs_gemini_home) try registry.resolveGeminiHome(allocator) else null;
    defer if (gemini_home) |path| allocator.free(path);

    switch (cmd) {
        .version => try cli.output.printVersion(),
        .help => |topic| switch (topic) {
            .top_level => try help_workflow.handleTopLevelHelp(allocator, gemini_home.?),
            else => try cli.help.printCommandHelp(topic),
        },
        .status => try auto.printStatus(allocator, gemini_home.?),
        .daemon => |opts| switch (opts.mode) {
            .watch => try auto.runDaemon(allocator, gemini_home.?),
            .once => try auto.runDaemonOnce(allocator, gemini_home.?),
        },
        .config => |opts| try config_workflow.handleConfig(allocator, gemini_home.?, opts),
        .list => |opts| try list_workflow.handleList(allocator, gemini_home.?, opts),
        .login => |opts| try login_workflow.handleLogin(allocator, gemini_home.?, opts),
        .import_auth => |opts| try import_workflow.handleImport(allocator, gemini_home.?, opts),
        .switch_account => |opts| try switch_workflow.handleSwitch(allocator, gemini_home.?, opts),
        .remove_account => |opts| try remove_workflow.handleRemove(allocator, gemini_home.?, opts),
        .clean => try clean_workflow.handleClean(allocator, gemini_home.?),
    }

    if (shouldReconcileManagedService(cmd)) {
        try auto.reconcileManagedService(allocator, gemini_home.?);
    }
}
