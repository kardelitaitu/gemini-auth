...
pub fn loadStoredSwitchSelectionDisplay(
    ...
    if (try registry.syncActiveAccountFromAuth(allocator, gemini_home, &latest)) {
...
pub fn loadStoredSwitchSelectionDisplayWithRefreshError(
    ...
    var loaded = try loadStoredSwitchSelectionDisplay(allocator, gemini_home, target, api_mode);
...
pub fn loadSwitchSelectionDisplay(
    ...
    var base = try registry.loadRegistry(allocator, gemini_home);
    defer base.deinit(allocator);

    var refreshed = try registry.loadRegistry(allocator, gemini_home);
    errdefer refreshed.deinit(allocator);
    _ = try registry.syncActiveAccountFromAuth(allocator, gemini_home, &refreshed);
    ...
    ensureForegroundNodeAvailableWithApiEnabled(
        ...
        error.OutOfMemory => return err,
            ...
            return loadStoredSwitchSelectionDisplayWithRefreshError(allocator, gemini_home, target, api_mode, err);
        },
    };
    ...
    var latest = try registry.loadRegistry(allocator, gemini_home);
    errdefer latest.deinit(allocator);
    var latest_changed = try registry.syncActiveAccountFromAuth(allocator, gemini_home, &latest);

    if (try mergeSwitchLiveRefreshIntoLatest(allocator, &latest, &base, &refreshed)) {
        latest_changed = true;
    }

    if (latest_changed) try registry.saveRegistry(allocator, gemini_home, &latest);
