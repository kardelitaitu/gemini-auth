...
pub fn removeSelectedAccountsAndPersist(
    ...
    var current_auth_state = try loadCurrentAuthState(allocator, gemini_home);
    ...
    if (replacement_account_key) |key| {
        if (allow_auth_file_update) {
            try registry.replaceActiveAuthWithAccountByKey(allocator, gemini_home, reg, key);
        } else {
            try registry.setActiveAccountKey(allocator, reg, key);
        }
    }

    try registry.removeAccounts(allocator, gemini_home, reg, selected);
    try reconcileActiveAuthAfterRemove(allocator, gemini_home, reg, allow_auth_file_update);
...
pub fn switchLiveRuntimeApplySelection(
    ...
    var reg = try registry.loadRegistry(allocator, runtime.gemini_home);
    defer reg.deinit(allocator);
    if (try registry.syncActiveAccountFromAuth(allocator, runtime.gemini_home, &reg)) {
        try registry.saveRegistry(allocator, runtime.gemini_home, &reg);
    }

    try registry.activateAccountByKey(allocator, runtime.gemini_home, &reg, account_key);
    try registry.saveRegistry(allocator, runtime.gemini_home, &reg);
...
pub fn removeLiveRuntimeApplySelection(
    ...
    try removeSelectedAccountsAndPersist(allocator, runtime.gemini_home, &reg, selected, false);
