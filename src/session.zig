...
pub fn scanLatestUsage(allocator: std.mem.Allocator, gemini_home: []const u8) !?registry.RateLimitSnapshot {
    const latest = try scanLatestUsageWithSource(allocator, gemini_home);
    if (latest == null) return null;
    allocator.free(latest.?.path);
    return latest.?.snapshot;
}

pub fn scanLatestUsageWithSource(allocator: std.mem.Allocator, gemini_home: []const u8) !?LatestUsage {
    var latest_rollout = (try scanLatestRolloutEventWithSource(allocator, gemini_home)) orelse return null;
...
pub fn scanLatestRolloutEventWithSource(allocator: std.mem.Allocator, gemini_home: []const u8) !?LatestRolloutEvent {
    const sessions_root = try std.fs.path.join(allocator, &[_][]const u8{ gemini_home, "sessions" });
...
pub fn scanLatestRolloutEventWithCache(
    ...
    if (cache.latest) |cached| {
        const stat = std.Io.Dir.cwd().statFile(app_runtime.io(), cached.path, .{}) catch |err| switch (err) {
            error.FileNotFound => return try refreshRolloutScanCache(allocator, gemini_home, cache, now_ns),
            else => return err,
        };
        ...
        if (current_mtime != cached.mtime) {
            ...
            return try refreshRolloutScanCache(allocator, gemini_home, cache, now_ns);
        }
...
fn refreshRolloutScanCache(
    ...
    const latest = try scanLatestRolloutEventWithSource(allocator, gemini_home);
