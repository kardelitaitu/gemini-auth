...
pub fn printStatus(allocator: std.mem.Allocator, gemini_home: []const u8) !void {
    const status = try getStatus(allocator, gemini_home);
    var stdout: io_util.Stdout = undefined;
    stdout.init();
    try writeStatusWithColor(stdout.out(), status, colorEnabled());
}

pub fn getStatus(allocator: std.mem.Allocator, gemini_home: []const u8) !Status {
