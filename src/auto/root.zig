...
const DaemonLock = struct {
    ...
    fn acquire(allocator: std.mem.Allocator, gemini_home: []const u8) !?DaemonLock {
        const path = try std.fs.path.join(allocator, &[_][]const u8{ gemini_home, "accounts", lock_file_name });
...
pub fn handleAutoCommand(allocator: std.mem.Allocator, gemini_home: []const u8, cmd: cli.types.AutoOptions) !void {
    switch (cmd) {
        .action => |action| switch (action) {
            .enable => try enable(allocator, gemini_home),
            .disable => try disable(allocator, gemini_home),
        },
        .configure => |opts| try configureThresholds(allocator, gemini_home, opts),
    }
}

pub fn handleApiCommand(allocator: std.mem.Allocator, gemini_home: []const u8, action: cli.types.ApiAction) !void {
...
pub fn reconcileManagedService(allocator: std.mem.Allocator, gemini_home: []const u8) !void {
    ...
    if (!reg.auto_switch.enabled) {
        try uninstallService(allocator, gemini_home);
        return;
    }
    ...
    const definition_matches = try currentServiceDefinitionMatches(allocator, gemini_home, managed_self_exe);
    if (!shouldEnsureManagedService(reg.auto_switch.enabled, runtime, definition_matches)) return;

    try installService(allocator, gemini_home, managed_self_exe);
}

pub fn runDaemon(allocator: std.mem.Allocator, gemini_home: []const u8) !void {
    try registry.ensureAccountsDir(allocator, gemini_home);
    var daemon_lock = (try DaemonLock.acquire(allocator, gemini_home)) orelse return;
    defer daemon_lock.release();
    var refresh_state = DaemonRefreshState{};
    defer refresh_state.deinit(allocator);

    while (true) {
        const keep_running = daemonCycle(allocator, gemini_home, &refresh_state) catch |err| blk: {
...
pub fn runDaemonOnce(allocator: std.mem.Allocator, gemini_home: []const u8) !void {
    try registry.ensureAccountsDir(allocator, gemini_home);
    var daemon_lock = (try DaemonLock.acquire(allocator, gemini_home)) orelse return;
    defer daemon_lock.release();
    ...
    var refresh_state = DaemonRefreshState{};
    defer refresh_state.deinit(allocator);
    _ = try daemonCycle(allocator, gemini_home, &refresh_state);
}
...
fn daemonCycleWithAccountNameFetcher(
    ...
    var reg = try refresh_state.ensureRegistryLoaded(allocator, gemini_home);
    if (!reg.auto_switch.enabled) return false;

    var changed = false;
    if (try refresh_state.syncActiveAuthIfChanged(allocator, gemini_home)) {
        changed = true;
    }

    if (changed) {
        try registry.saveRegistry(allocator, gemini_home, reg);
        try refresh_state.refreshTrackedFileMtims(allocator, gemini_home);
        changed = false;
    }

    if (try refreshActiveAccountNamesForDaemonWithFetcher(allocator, gemini_home, reg, refresh_state, account_name_fetcher)) {
        changed = true;
    }
    try refresh_state.reloadRegistryStateIfChanged(allocator, gemini_home);
    reg = refresh_state.currentRegistry();
    if (!reg.auto_switch.enabled) return true;

    if (try refreshActiveUsageForDaemon(allocator, gemini_home, reg, refresh_state)) {
        changed = true;
    }
    const active_idx_before = if (reg.active_account_key) |account_key|
        ...
        null;
    const auto_switch_attempt = try maybeAutoSwitchForDaemonWithUsageFetcher(allocator, gemini_home, reg, refresh_state, usage_api.fetchUsageForAuthPathDetailed);
...
fn daemonCycle(allocator: std.mem.Allocator, gemini_home: []const u8, refresh_state: *DaemonRefreshState) !bool {
    return daemonCycleWithAccountNameFetcher(allocator, gemini_home, refresh_state, fetchActiveAccountNames);
}
...
pub fn daemonCycleWithAccountNameFetcherForTest(
    ...
    return daemonCycleWithAccountNameFetcher(allocator, gemini_home, refresh_state, account_name_fetcher);
}

fn enable(allocator: std.mem.Allocator, gemini_home: []const u8) !void {
    const self_exe = try std.process.executablePathAlloc(app_runtime.io(), allocator);
    defer allocator.free(self_exe);
    const managed_self_exe = try managedServiceSelfExePath(allocator, self_exe);
    defer allocator.free(managed_self_exe);
    try enableWithServiceHooks(allocator, gemini_home, managed_self_exe, installService, uninstallService);
}
...
pub fn enableWithServiceHooksAndPreflight(
    ...
    errdefer {
        ...
        registry.saveRegistry(allocator, gemini_home, &reg) catch {};
    }
    ...
    errdefer uninstaller(allocator, gemini_home) catch {};
    try installer(allocator, gemini_home, self_exe);
...
fn printAutoEnableUsageNote(api_enabled: bool) !void {
    ...
    if (api_enabled) {
        ...
        try out.writeAll("Tip: run `gemini-auth config api enable` for the most accurate switching decisions.\n");
    }
    try out.flush();
}

fn disable(allocator: std.mem.Allocator, gemini_home: []const u8) !void {
    var reg = try registry.loadRegistry(allocator, gemini_home);
    defer reg.deinit(allocator);
    reg.auto_switch.enabled = false;
    try registry.saveRegistry(allocator, gemini_home, &reg);
...
fn configureThresholds(allocator: std.mem.Allocator, gemini_home: []const u8, opts: cli.types.AutoThresholdOptions) !void {
    var reg = try registry.loadRegistry(allocator, gemini_home);
    defer reg.deinit(allocator);
    applyThresholdConfig(&reg.auto_switch, opts);
    try registry.saveRegistry(allocator, gemini_home, &reg);
    try printStatus(allocator, gemini_home);
}
