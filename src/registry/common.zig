...
pub fn freeAccountRecord(allocator: std.mem.Allocator, rec: *const AccountRecord) void {
    ...
    if (rec.name) |name| allocator.free(name);
...
pub fn resolveExistingGeminiHomeOverride(allocator: std.mem.Allocator, path: []const u8) ![]u8 {
...
pub fn resolveGeminiHomeFromEnv(
    allocator: std.mem.Allocator,
    gemini_home_override: ?[]const u8,
    home: ?[]const u8,
    user_profile: ?[]const u8,
) ![]u8 {
    if (gemini_home_override) |path| {
        if (path.len != 0) return try resolveExistingGeminiHomeOverride(allocator, path);
    }
    if (home) |path| {
        if (path.len != 0) return try std.fs.path.join(allocator, &[_][]const u8{ path, ".gemini" });
    }
...
pub fn resolveGeminiHome(allocator: std.mem.Allocator) ![]u8 {
    const gemini_home_override = try getNonEmptyEnvVarOwned(allocator, "GEMINI_HOME");
    ...
    return try resolveGeminiHomeFromEnv(allocator, gemini_home_override, home, user_profile);
}
...
pub fn ensureAccountsDir(allocator: std.mem.Allocator, gemini_home: []const u8) !void {
    const accounts_dir = try std.fs.path.join(allocator, &[_][]const u8{ gemini_home, "accounts" });
    defer allocator.free(accounts_dir);
    try ensurePrivateDir(accounts_dir);
}

pub fn registryPath(allocator: std.mem.Allocator, gemini_home: []const u8) ![]u8 {
    return try std.fs.path.join(allocator, &[_][]const u8{ gemini_home, "accounts", "registry.json" });
}
...
pub fn accountSnapshotFileName(allocator: std.mem.Allocator, account_key: []const u8) ![]u8 {
    ...
    return try std.mem.concat(allocator, u8, &[_][]const u8{ key, ".oauth_creds.json" });
}

pub fn accountAuthPath(allocator: std.mem.Allocator, gemini_home: []const u8, account_key: []const u8) ![]u8 {
    const filename = try accountSnapshotFileName(allocator, account_key);
    defer allocator.free(filename);
    return try std.fs.path.join(allocator, &[_][]const u8{ gemini_home, "accounts", filename });
}

pub fn legacyAccountAuthPath(allocator: std.mem.Allocator, gemini_home: []const u8, email: []const u8) ![]u8 {
    const key = try encodedFileKey(allocator, email);
    defer allocator.free(key);
    const filename = try std.mem.concat(allocator, u8, &[_][]const u8{ key, ".oauth_creds.json" });
    defer allocator.free(filename);
    return try std.fs.path.join(allocator, &[_][]const u8{ gemini_home, "accounts", filename });
}

pub fn activeAuthPath(allocator: std.mem.Allocator, gemini_home: []const u8) ![]u8 {
    return try std.fs.path.join(allocator, &[_][]const u8{ gemini_home, "oauth_creds.json" });
}
