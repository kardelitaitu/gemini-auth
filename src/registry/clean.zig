...
pub fn backupDir(allocator: std.mem.Allocator, gemini_home: []const u8) ![]u8 {
    return try std.fs.path.join(allocator, &[_][]const u8{ gemini_home, "accounts" });
}
...
pub fn resolveStrictAccountAuthPath(
    ...
    const path = try accountAuthPath(allocator, gemini_home, account_key);
    if (std.Io.Dir.cwd().openFile(app_runtime.io(), path, .{})) |file| {
        file.close(app_runtime.io());
        return path;
    } else |err| {
...
pub fn cleanAccountsBackupsWithLoader(allocator: std.mem.Allocator, gemini_home: []const u8, load_registry: anytype) !CleanSummary {
    const dir = try backupDir(allocator, gemini_home);
    defer allocator.free(dir);
    const reg_path = try registryPath(allocator, gemini_home);
    ...
    const auth_before = try countBackupsByBaseName(allocator, dir, "oauth_creds.json");
    const registry_before = try countBackupsByBaseName(allocator, dir, "registry.json");

    try pruneBackups(allocator, dir, "oauth_creds.json", 0);
    try pruneBackups(allocator, dir, "registry.json", 0);

    const auth_after = try countBackupsByBaseName(allocator, dir, "oauth_creds.json");
    ...
    var reg = try load_registry(allocator, gemini_home);
...
pub fn backupAuthIfChanged(
    ...
    if (!(try filesEqual(allocator, current_auth_path, new_auth_path))) {
        ...
        const backup = try makeBackupPath(allocator, dir, "oauth_creds.json");
        defer allocator.free(backup);
        try copyManagedFile(current_auth_path, backup);
        try pruneBackups(allocator, dir, "oauth_creds.json", max_backups);
    }
}
