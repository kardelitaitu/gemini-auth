...
fn resolveLegacySnapshotPathForEmail(
    ...
    const legacy_path = try legacyAccountAuthPath(allocator, gemini_home, email);
    ...
    const accounts_dir = try backupDir(allocator, gemini_home);
    ...
    const active_path = try activeAuthPath(allocator, gemini_home);
...
fn migrateLegacyRecord(
    ...
    const legacy_path = try resolveLegacySnapshotPathForEmail(allocator, gemini_home, legacy.email);
    ...
    const new_path = try accountAuthPath(allocator, gemini_home, rec.account_key);
    ...
    const old_legacy_path = try legacyAccountAuthPath(allocator, gemini_home, legacy.email);
...
fn loadLegacyRegistryV2(
    ...
    for (legacy_accounts.items) |*legacy| {
        try migrateLegacyRecord(allocator, gemini_home, &reg, legacy_active_email, legacy);
    }
...
fn logUnsupportedRegistryVersion(version_value: u32) void {
    ...
    std.log.err(
        "registry schema_version {d} is newer than this gemini-auth binary supports (max {d}); upgrade gemini-auth",
        .{ version_value, current_schema_version },
    );
}

pub fn loadRegistry(allocator: std.mem.Allocator, gemini_home: []const u8) !Registry {
    const path = try registryPath(allocator, gemini_home);
    ...
    var reg = switch (schema_version) {
        2 => try loadLegacyRegistryV2(allocator, gemini_home, root_obj),
        3, 4 => try loadCurrentRegistry(allocator, root_obj),
        else => {
            std.log.err(
                "registry schema_version {d} is older than the minimum supported {d}; use an intermediate gemini-auth release or import --purge",
    ...
    if (needs_rewrite) {
        try saveRegistry(allocator, gemini_home, &reg);
    }
