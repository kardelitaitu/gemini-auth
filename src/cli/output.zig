...
pub fn printSwitchRequiresTtyError() !void {
    ...
    try writeHintPrefixTo(out, use_color);
    try out.writeAll(" Run `gemini-auth switch` in a terminal, or narrow `gemini-auth switch <alias|email|display-number|query>` to one account.\n");
    try out.flush();
}
...
pub fn printListRequiresTtyError() !void {
    ...
    try writeHintPrefixTo(out, use_color);
    try out.writeAll(" Run `gemini-auth list --live` in a terminal.\n");
    try out.flush();
}
...
pub fn printRemoveRequiresTtyError() !void {
    ...
    try writeHintPrefixTo(out, use_color);
    try out.writeAll(" Use `gemini-auth remove <alias|email|display-number|query>...` or `gemini-auth remove --all` instead.\n");
    try out.flush();
}
...
pub fn writeCodexLoginLaunchFailureHintTo(out: *std.Io.Writer, err_name: []const u8, use_color: bool) !void {
    ...
    if (std.mem.eql(u8, err_name, "FileNotFound")) {
        ...
        try writeHintPrefixTo(out, use_color);
        try out.writeAll(" Try running `gemini login` manually, then retry your command.\n");
    }
}
