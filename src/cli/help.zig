...
fn writeCommandHelpHeader(out: *std.Io.Writer, use_color: bool, topic: HelpTopic) !void {
    if (use_color) try out.writeAll(style.ansi.cyan);
    try out.print("gemini-auth {s}", .{commandNameForTopic(topic)});
    if (use_color) try out.writeAll(style.ansi.reset);
    try out.writeAll("\n");
...
fn writeUsageLines(out: *std.Io.Writer, topic: HelpTopic) !void {
    switch (topic) {
        .top_level => {
            try out.writeAll("  gemini-auth <command>\n");
            try out.writeAll("  gemini-auth --help\n");
            try out.writeAll("  gemini-auth help <command>\n");
        },
        .list => try out.writeAll("  gemini-auth list [--live] [--api|--skip-api]\n"),
        .status => try out.writeAll("  gemini-auth status\n"),
        .login => {
            try out.writeAll("  gemini-auth login\n");
            try out.writeAll("  gemini-auth login --device-auth\n");
        },
        .import_auth => {
            try out.writeAll("  gemini-auth import <path> [--alias <alias>]\n");
            try out.writeAll("  gemini-auth import --cpa [<path>] [--alias <alias>]\n");
            try out.writeAll("  gemini-auth import --purge [<path>]\n");
        },
        .switch_account => {
            try out.writeAll("  gemini-auth switch [--live] [--api|--skip-api]\n");
            try out.writeAll("  gemini-auth switch <alias|email|display-number|query>\n");
        },
        .remove_account => {
            try out.writeAll("  gemini-auth remove [--live] [--api|--skip-api]\n");
            try out.writeAll("  gemini-auth remove <alias|email|display-number|query>...\n");
            try out.writeAll("  gemini-auth remove --all\n");
        },
        .clean => try out.writeAll("  gemini-auth clean\n"),
        .config => {
            try out.writeAll("  gemini-auth config auto enable\n");
            try out.writeAll("  gemini-auth config auto disable\n");
            try out.writeAll("  gemini-auth config auto --5h <percent> [--weekly <percent>]\n");
            try out.writeAll("  gemini-auth config auto --weekly <percent>\n");
            try out.writeAll("  gemini-auth config api enable\n");
            try out.writeAll("  gemini-auth config api disable\n");
            try out.writeAll("  gemini-auth config live --interval <seconds>\n");
        },
        .daemon => {
            try out.writeAll("  gemini-auth daemon --watch\n");
            try out.writeAll("  gemini-auth daemon --once\n");
        },
    }
}
...
pub fn helpCommandForTopic(topic: HelpTopic) []const u8 {
    return switch (topic) {
        .top_level => "gemini-auth --help",
        .list => "gemini-auth list --help",
        .status => "gemini-auth status --help",
        .login => "gemini-auth login --help",
        .import_auth => "gemini-auth import --help",
        .switch_account => "gemini-auth switch --help",
        .remove_account => "gemini-auth remove --help",
        .clean => "gemini-auth clean --help",
        .config => "gemini-auth config --help",
        .daemon => "gemini-auth daemon --help",
    };
}
...
fn writeOptionLines(out: *std.Io.Writer, topic: HelpTopic) !void {
    switch (topic) {
        ...
        .login => {
            try out.writeAll("  --device-auth   Run `gemini login --device-auth` before adding the account.\n");
        },
        .import_auth => {
...
fn writeExampleLines(out: *std.Io.Writer, topic: HelpTopic) !void {
    switch (topic) {
        .top_level => {
            try out.writeAll("  gemini-auth list\n");
            try out.writeAll("  gemini-auth import /path/to/auth.json --alias personal\n");
            try out.writeAll("  gemini-auth config auto enable\n");
        },
        .list => {
            try out.writeAll("  gemini-auth list\n");
            try out.writeAll("  gemini-auth list --live\n");
            try out.writeAll("  gemini-auth list --api\n");
            try out.writeAll("  gemini-auth list --skip-api\n");
        },
        .status => try out.writeAll("  gemini-auth status\n"),
        .login => {
            try out.writeAll("  gemini-auth login\n");
        ...
        .import_auth => {
            try out.writeAll("  gemini-auth import /path/to/auth.json --alias personal\n");
            try out.writeAll("  gemini-auth import --cpa /path/to/token.json --alias work\n");
            try out.writeAll("  gemini-auth import --purge\n");
        },
        .switch_account => {
            try out.writeAll("  gemini-auth switch\n");
            try out.writeAll("  gemini-auth switch --live\n");
            try out.writeAll("  gemini-auth switch --api\n");
            try out.writeAll("  gemini-auth switch --skip-api\n");
            try out.writeAll("  gemini-auth switch personal\n");
            try out.writeAll("  gemini-auth switch john@example.com\n");
            try out.writeAll("  gemini-auth switch 02\n");
            try out.writeAll("  gemini-auth switch work\n");
        },
        .remove_account => {
            try out.writeAll("  gemini-auth remove\n");
            try out.writeAll("  gemini-auth remove --live\n");
            try out.writeAll("  gemini-auth remove --api\n");
            try out.writeAll("  gemini-auth remove --skip-api\n");
            try out.writeAll("  gemini-auth remove 01 03\n");
            try out.writeAll("  gemini-auth remove work personal\n");
            try out.writeAll("  gemini-auth remove john@example.com jane@example.com\n");
            try out.writeAll("  gemini-auth remove --all\n");
        },
        .clean => try out.writeAll("  gemini-auth clean\n"),
        .config => {
            try out.writeAll("  gemini-auth config auto --5h 12 --weekly 8\n");
            try out.writeAll("  gemini-auth config api enable\n");
            try out.writeAll("  gemini-auth config live --interval 60\n");
        },
        .daemon => {
            try out.writeAll("  gemini-auth daemon --watch\n");
            try out.writeAll("  gemini-auth daemon --once\n");
