...
fn runCliWithIsolatedHome(
    ...
    _ = env_map.swapRemove("GEMINI_HOME");
...
test "Scenario: Given GEMINI_HOME override when running login then it stores auth state under the override root" {
    const gpa = std.testing.allocator;
    const project_root = try projectRootAlloc(gpa);
    defer gpa.free(project_root);
    try buildCliBinary(gpa, project_root);
...
test "Scenario: Given failed device auth login with existing auth json when running login then it forwards the flag and does not mutate the registry" {
    ...
    try std.testing.expectError(error.FileNotFound, tmp.dir.openFile(".gemini/accounts/registry.json", .{}));
...
// the old email-keyed registry and snapshot exist under ~/.gemini/accounts before the new binary runs.
test "Scenario: Given upgrade from v0.1.x to v0.2 with legacy accounts data when list runs then cli migrates registry and keeps account usable" {
    ...
    const legacy_rel = try fs.path.join(gpa, &[_][]const u8{ ".gemini", "accounts", legacy_name });
    defer gpa.free(legacy_rel);
    try tmp.dir.writeFile(.{ .sub_path = legacy_rel, .data = auth_json });

    try tmp.dir.writeFile(.{
        .sub_path = ".gemini/accounts/registry.json",
        .data =
        \\{
        \\  "version": 2,
...
test "Scenario: Given purge with no recoverable active auth when running import then it activates the first rebuilt account and backs up auth json" {
    ...
    try std.testing.expectEqual(@as(usize, 1), try countAuthBackups(tmp.dir, ".gemini/accounts"));

    var backup_name: ?[]u8 = null;
    defer if (backup_name) |name| gpa.free(name);

    var accounts = try tmp.dir.openDir(".gemini/accounts", .{ .iterate = true });
    defer accounts.close();
    var it = accounts.iterate();
    while (try it.next()) |entry| {
        if (entry.kind != .file) continue;
    ...
    const backup_rel = try fs.path.join(gpa, &[_][]const u8{ ".gemini", "accounts", backup_name.? });
...
test "Scenario: Given active account removal with a replacement when running remove then it does not recreate a backup for the deleted auth" {
    ...
    try std.testing.expectEqual(@as(usize, 0), try countAuthBackups(tmp.dir, ".gemini/accounts"));
}
